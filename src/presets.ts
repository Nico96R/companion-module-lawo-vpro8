...
export function GetPresetsList(state: Vpro8State): CompanionPresetDefinitions {
	...
	presets['undo'] = {
		...
		type: 'button',
		style: {
			text: 'Undo\n$(vpro8:selected_target_undo_source)', //TODO: use dynamic module name
			size: '18',
		...
		for (const output of state.iterateOutputs(matrix.id)) {
			if (output != undefined && output.active) {
				presets[`select_destination_${matrix.label}_${output.id}`] = {
					...
					type: 'button',
					style: {
						text: `$(vpro8:output_${matrix.label}_${output.id})`,
						size: '18',
						color: combineRgb(255, 255, 255),
						bgcolor: combineRgb(0, 0, 0),
					},
		...
		for (const input of state.iterateInputs(matrix.id)) {
			if (input != undefined && input.active) {
				presets[`route_source_${matrix.label}_${input.id}`] = {
					...
					type: 'button',
					style: {
						text: `$(vpro8:input_${matrix.label}_${input.id})`,
						size: '18',
						color: combineRgb(255, 255, 255),
						bgcolor: combineRgb(0, 0, 0),
					},
        ...
        for (const output of state.iterateOutputs(matrix.id)) {
          for (const input of state.iterateInputs(matrix.id)) {
            ...
            presets[`output_${output.id}_${input.id}_momentary`] = {
              ...
              type: 'button',
              style: {
                text: `$(vpro8:input_${input.id + 1}) (mom.)`,
                size: '18',
                color: combineRgb(255, 255, 255),
                bgcolor: combineRgb(0, 0, 0),
              },
