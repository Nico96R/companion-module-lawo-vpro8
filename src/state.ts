...
import { Vpro8Instance } from './index'
...
const videoPath = '1.10.2'
const audioPath = '1.11.2'
const videoQuadPath = '1.12.2'
const audioQuadPath = '1.13.2'
...
export class Vpro8State {
	...
	constructor(self: Vpro8Instance) {
		this.self = self
		this.selected = {
			source: -1,
			target: -1,
		...
		this.matrices = [
			...
			{ id: 2, label: 'video_quad', path: videoQuadPath, inputs: [], outputs: [] },
			{ id: 3, label: 'audio_quad', path: audioQuadPath, inputs: [], outputs: [] },
		]
	}

	public updateCounts(config: Vpro8Config): void {
	...
	/**
	 * Initially reads all wanted values from the vPro8.
	 * Callbacks are given for wanted updates.
	 */
	public async subscribeVpro8(): Promise<void> {
